    ...
    >
      ...
      >
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-2">
              <button
                ...
                className={`py-3 px-4 rounded-xl text-xs font-bold border-2 transition-all ${sortOrder === 'asc' ? 'border-blue-600 bg-blue-50 text-blue-700' : 'border-slate-100 bg-slate-50 text-slate-600'}`}
              >
                Low to High
              </button>
              <button
                onClick={() => setSortOrder(sortOrder === 'desc' ? null : 'desc')}
                className={`py-3 px-4 rounded-xl text-xs font-bold border-2 transition-all ${sortOrder === 'desc' ? 'border-blue-600 bg-blue-50 text-blue-700' : 'border-slate-100 bg-slate-50 text-slate-600'}`}
              >
                High to Low
              </button>
            </div>
              ...
              {priceRanges.map((range) => {
                ...
                return (
                  <button
                    ...
                    className={`flex justify-between items-center py-3.5 px-4 rounded-xl text-sm font-bold border-2 transition-all ${isSelected ? 'border-blue-600 bg-blue-50 text-blue-700' : 'border-slate-100 bg-slate-50 text-slate-600 hover:border-slate-200'}`}
                  >
        ...
        <div className="flex gap-3 pt-2">
          ...
          <button
            onClick={onClose}
            className="flex-1 py-3.5 rounded-xl bg-blue-600 text-white text-sm font-black uppercase shadow-lg shadow-blue-100 hover:bg-blue-700 transition-all"
          >
            Apply
          </button>
        </div>
